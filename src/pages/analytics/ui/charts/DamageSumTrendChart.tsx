...
// Динамика суммы ущерба по месяцам — две линии (по автоколонне).
export function DamageSumTrendChart({ data, rowsA, rowsB }: Props) {
  const chartData = data.trendMonths.map((ym, index) => ({
    ym,
    label: formatMonthShortLabel(ym),
    a: data.monthlyA[index]?.sumDamage ?? 0,
    b: data.monthlyB[index]?.sumDamage ?? 0,
  }))
